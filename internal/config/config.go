...
type Warm struct {
	...
	// CacheHash pins the size set the requests are routed through. Normally it
	// is discovered from the cache directory, or recovered with one request,
	// so this exists for pinning a specific set by hand.
	//
	// It does not have to be the only set the theme asks for: a request
	// regenerates the whole family either way (see internal/warm). What it does
	// have to be is live, meaning a set the theme still asks for, or the
	// requested path never appears. A wrong value is therefore corrected at
	// runtime rather than fatal.
	CacheHash string `yaml:"cacheHash"`
	// HashFile is a snapshot of the size sets, written by
	// `cache clean --save-hashes` before the tree is emptied. It saves the one
	// request a cold start would otherwise spend recovering a live set.
	HashFile string `yaml:"hashFile"`
	// Loopback sends the run's traffic to 127.0.0.1 instead of to the address
	// the storefront's domain resolves to, while leaving the Host header and
	// the TLS server name alone. That is the right setting for a run on the web
	// server itself: it costs no external bandwidth, and unlike an /etc/hosts
	// entry it keeps working on a host that serves several shops.
	Loopback bool `yaml:"loopback"`
	// Resolve lists "host:port:address" overrides for deployments where
	// loopback is not enough — a Unix socket front end, a container network, a
	// second interface. Each entry replaces the address dialed for one
	// host and port, and nothing else about the request.
	Resolve []string `yaml:"resolve"`
	// InsecureSkipVerify accepts any TLS certificate, which is what makes an
	// https storefront warmable when its certificate is self-signed for an
	// internal name. Go rejects a certificate that has no subject alternative
	// name even if a client would trust it, so shops like that are otherwise
	// unreachable. Opt-in, and reported in the run's output.
	InsecureSkipVerify bool `yaml:"insecureSkipVerify"`
	// Concurrency is the number of requests in flight.
	Concurrency int `yaml:"concurrency"`
	// Timeout bounds a single request.
	Timeout Duration `yaml:"timeout"`
	// Method is "get" or "head"; empty means "get". A missing variant is
	// generated by either, so head saves the transfer; get is the default
	// because a caching proxy may answer a head from the edge without
	// generating anything.
